...
CREATE DEFINER = `root`@`localhost` TRIGGER `updatefulltextsearchstory` AFTER UPDATE ON `story` FOR EACH ROW UPDATE fulltextsearchstory set TextDescription=NEW.TextDescription, Title=NEW.Title where Id = OLD.Id $$
...
CREATE DEFINER = `root`@`localhost` TRIGGER `deletefulltextsearchstory` AFTER DELETE ON `story` FOR EACH ROW DELETE FROM fulltextsearchstory where Id = OLD.Id $$
...
CREATE DEFINER = `root`@`localhost` TRIGGER `updatefulltextsearchcomment` AFTER UPDATE ON `storycomment` FOR EACH ROW UPDATE fulltextsearchcomment set TextBody=NEW.TextBody where Id = OLD.Id $$
...
CREATE DEFINER = `root`@`localhost` TRIGGER `deletefulltextsearchcomment` AFTER DELETE ON `storycomment` FOR EACH ROW DELETE FROM fulltextsearchcomment where Id = OLD.Id $$
