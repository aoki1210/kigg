...
namespace Kigg.Infrastructure.EF.Test
{
    ...
    public class UserFixture : EfBaseFixture
    {
        ...
        {
            _user = new User();
            _mockedTags = new Mock<EntityCollection<ITag, Tag>>(_user.UserTagsInternal);
            _user.UserTags = _mockedTags.Object;
        }
        ...
        {
            _user.UserScoreInternal.Clear();

            _user.IncreaseScoreBy(10, UserAction.AccountActivated);

            Assert.Equal(1, _user.UserScoreInternal.Count);
        }
